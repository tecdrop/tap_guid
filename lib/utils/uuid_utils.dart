...
String formatUuid(String uuid, {UuidFormat format = .standard, bool uppercase = false}) {
  // First apply uppercase if needed (except for base64 formats)
  final processedUuid = (uppercase && format != .base64 && format != .base64url)
      ? uuid.toUpperCase()
      : uuid;

  // Then apply the format to the already-processed UUID
  return switch (format) {
    .standard => processedUuid,
    .digits => processedUuid.replaceAll(RegExp(r'-'), ''),
    .braces => '{$processedUuid}',
    .parentheses => '($processedUuid)',
    .urn => 'urn:uuid:$processedUuid',
    .base64 => base64.encode(Uuid.parse(uuid)),
    .base64url => base64Url.encode(Uuid.parse(uuid)),
  };
}
...
Color getUuidColor(String uuid) {
  final hash = _fnvHash32(uuid);
  final generatedColor = Color(hash | 0xFF000000);

  // Simulate mixing by averaging the generated color with a medium gray (#666666)
  const gray = Color(0xFF666666);
  final red = (((generatedColor.r * 255) + (gray.r * 255)) / 2).round();
  final green = (((generatedColor.g * 255) + (gray.g * 255)) / 2).round();
  final blue = (((generatedColor.b * 255) + (gray.b * 255)) / 2).round();
